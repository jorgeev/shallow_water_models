...
@nb.njit(nb.float32[:](nb.float32[:], nb.int8, nb.int8, nb.int8))
def perturbate(h1, ii, DT, period):
        h1[0] = np.sin((ii)*2*np.pi*DT/period)
        h1[1] = np.sin((ii-1)*2*np.pi*DT/period)
        return h1

@nb.njit(nbtypes.Tuple([nb.float32[:], nb.float32[:]])(nb.float32[:],nb.float32[:]))
...
@nb.njit(nb.float32[:](nb.float32[:], nb.float32[:], nb.float32[:], nb.float32, nb.int8), parallel=True)
